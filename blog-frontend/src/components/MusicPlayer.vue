      ...
      <div v-show="expanded" class="player-card glass">
        ...
        <div v-if="searchResults.length" class="search-results">
          <div v-for="song in searchResults" :key="song.id" class="result-item" @click="playSong(song)">
            <img :src="getCoverUrl(song.album?.picId || song.al?.picUrl)" @error="handleImgError" />
            <div class="info">
        ...
        <div v-if="currentSong" class="now-playing">
          <img :src="getCoverUrl(currentSong.al?.picUrl)" class="cover" :class="{ spinning: musicStore.isPlaying }" @error="handleImgError" />
          <div class="info">
...
const handleSearch = async () => {
  ...
  try {
    const res = await searchSongs(searchKeyword.value)
...
const getCoverUrl = (url) => url ? url + '?param=100y100' : defaultCover
