    ...
    <div class="glass rounded-xl p-4 sm:p-6 mb-6">
      <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 mb-4">
        ...
        <div class="flex-1 text-center sm:text-left">
          ...
          <p class="text-gray-500 mt-1 text-sm">{{ profile.user?.bio || '暂无简介' }}</p>
          <div class="flex justify-center sm:justify-start gap-4 mt-2 text-sm text-gray-600 dark:text-gray-400">
            <span>粉丝 {{ profile.followerCount }}</span>
            <span>关注 {{ profile.followingCount }}</span>
          </div>
        </div>
        <div v-if="userStore.isLoggedIn && userStore.user?.id === Number(route.params.id)" class="flex-shrink-0">
          <el-button size="small" @click="showEditDialog = true">编辑资料</el-button>
        </div>
        <div v-else-if="userStore.isLoggedIn && userStore.user?.id !== Number(route.params.id)" class="flex gap-2 flex-shrink-0">
          <el-button type="primary" size="small" @click="handleFollow">{{ isFollowing ? '取消关注' : '关注' }}</el-button>
          <el-button size="small" @click="router.push(`/chat?userId=${route.params.id}`)">私信</el-button>
        </div>
      </div>
    </div>
